...
解锁并切换不同树种，每种树都有专属的主题装饰可以获得和装备——鸟居、石灯笼、风铃、竹篱笆、秋千等等。

<img src="assets/species.zh-CN.png" alt="苹果树、樱花树、仙人掌——还有一个神秘树种将在节日季登场" width="100%" />

> 🎄 **节日季有东西要来了……** 第四个树种已经在我们的温室里悄悄发芽。Star 本仓库，第一时间看揭晓。
...
想比一比？加入[全球排行榜](https://www.tokenforest.com.au)，看看你的森林在全世界排第几。完全自愿：只上传昵称、地区、树的 token 总量和成长阶段——退出排行榜时，你的记录会被彻底删除。

<img src="assets/leaderboard.zh-CN.png" alt="全球排行榜——按玩家养进树里的 token 总量排名" width="100%" />

### 📊 数据面板 —— 开发中

给你的森林配一张"成绩单"：种植天数、累计 token、历史成长曲线、Claude / Codex 各模型用量拆分，以及一份**完全离线**、按内置价目表计算的真实成本估算。
...
- **说你的语言** —— 应用支持中文和 English；官网支持中 / 英 / 日 / 韩。
...
| --- | --- |
| 读取 Claude Code / Codex 本来就存在本机的用量日志里的 token **数量** | 读取你的代码、提示词或对话内容 |
| 完全在本机计算成长、统计和成本 | 发送任何遥测或分析数据 |
| 把状态存在本地小文件里 | 连接网络——除非你主动开启排行榜 |
| （仅排行榜、仅自愿）上传昵称、地区、token 总量、成长阶段 | 上传除此之外的任何东西 |
...
| | |
| --- | --- |
| ✅ 已上线 | 实时 token 追踪（Claude Code + Codex）· 8 阶段成长 · 3 个树种与装饰 · 果实与商店 · 气泡 · 胶囊模式 · 自愿排行榜 · Windows & macOS |
| 🚧 进行中 | 数据面板 · 一键安装包 · **🎄 某个节日限定树种** |
...
不会。它只读本地用量日志里的 token *数量*——永远不碰文件内容、提示词或对话。详见[隐私文档](docs/PRIVACY.zh-CN.md)。
...
Token Forest 由一个 token 烧得*非常多*的小团队打造——所以我们很确定这棵树真的会长大。
...
© 2026 Token Forest 团队。所有美术与品牌素材保留一切权利——见 [LICENSE.md](LICENSE.zh-CN.md)。
